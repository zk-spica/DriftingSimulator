...
//keystatus: 1 - pressed; 0 - released
int keyStatus[300];

static void KeyboardCB(unsigned char Key, int x, int y)
{
	if (keyStatus[Key] == 1) return;
	keyStatus[Key] = 1;
	...
	if (
        ...
        printf("Unimplemented GLUT key %d\n", Key);        
...
static void MouseWheelCB(int wheel, int direction, int x, int y)
{
	...
	s_pCallbacks->MouseWheelCB(direction);
}
...
void GLUTBackendInit(int argc, char** argv, bool WithDepth, bool WithStencil)
{
    ...
    uint DisplayMode = GLUT_DOUBLE|GLUT_RGBA|GLUT_ACCUM|GLUT_ALPHA;
