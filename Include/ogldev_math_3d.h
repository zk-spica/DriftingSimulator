...
struct Vector2f
{
	...
	Vector2f Rotate(double a) const
	{
		return Vector2f(x*cos(a)-y*sin(a), x*sin(a)+y*cos(a));
	}

	Vector2f Project(Vector2f &d)
	{
		return d*((x*d.x+y*d.y)/(d.x*d.x+d.y*d.y));
	}
...
inline Vector2f operator-(const Vector2f& l)
{
    Vector2f Ret(-l.x, -l.y);
    return Ret;
}

inline Vector2f operator+(const Vector2f& l, const Vector2f& r)
{
    Vector2f Ret(l.x+r.x, l.y+r.y);
    return Ret;
}
